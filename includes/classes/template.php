...
//    $sn_page_name ? lng_load_i18n($sn_mvc['i18n'][$sn_page_name]) : false;
  ...
  /**
   * Список добавленных файлов шаблонов
   *
   * @var string[] $files_added
   *
   * (c) Gorlum 2015
   */
  public $files_added = array();

  /**
   * @param null|string|string[] $files
   * @param bool|false $template_path
   *
   * (c) Gorlum 2015
   */
  public function __construct($files = null, $template_path = false) {
    $this->set_custom_template(($template_path ? $template_path : SN_ROOT_PHYSICAL . 'design/templates/') . DEFAULT_TEMPLATE . '/', DEFAULT_TEMPLATE, TEMPLATE_DIR);

    $this->addFileList($files);
  }

  /**
   * Добавляет еще файлы шаблонов к текущему темплейту
   *
   * @param string|string[] $files
   *
   * (c) Gorlum 2015
  ...
  public function addFileList($files) {
    ...
    $this->files_added = array_merge($this->files_added, $files);
  }

  /**
   * Рендерит страницу
   */
  public function displayPageFull() {
    $this->files_added = array_merge(
      array('_global_header' => '_global_header' . TEMPLATE_EXTENSION),
      $this->files_added,
      array('_global_footer' => '_global_footer' . TEMPLATE_EXTENSION)
    );

    $this->set_filenames($this->files_added);
...
//          'USER_AUTHLEVEL'=> isset($user['authlevel']) ? $user['authlevel'] : -1,
    ...
    foreach($this->files as $section => $filename) {
